...
def main():
    ...
    st.set_page_config(page_title="Farmer Dashboard ",
                       page_icon=":bar_chart:",
                       layout="wide")
    st.markdown(
    """
    ...
    # Filter the dataframe based on selected Talukas
    filtered_df = df[df['Taluka'].isin(selected_Taluka)]
    ...
    filtered_df = df[df['Crop Name'].isin(selected_crop)]
   ...
   # Pie chart for Crop distribution using Plotly
    fig = px.pie(filtered_df, names='Crop Name', title=f'Distribution of Crops in {selected_Taluka} Taluka')
